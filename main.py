...
def get_info():
    ...
    n2=name[1].replace(" ","")
    if n1=="" or n2=="":
        return redirect(url_for('fill_details'))   #if we havent filled the details and submitted then open the fill details page again
    else:
        ...
        print(data)
        # if data[0][0]>5:  #if we want only 5 users in dataset
        ...
        ######### Insert name ie in a column in excel sheet ##############
        ...
        conn.close()
        # data=cursor.fetchall() 
        return render_template('make_dataset.html')   #make_dataset.html used for generating images for training
